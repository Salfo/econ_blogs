...
url = 'http://econbrowser.com/'
b_6 <- extract_links(url = url,
                         l_xpath = '//*[(@id = "content")] | //*[contains(concat( " ", @class, " " ), concat( " ", "entry-title", " " ))]',
                         d_xpath = '//*[contains(concat( " ", @class, " " ), concat( " ", "entry-date", " " ))]')
Links <- unlist(b_6$links)[seq(2, 20, 2)]
b_6 <- data.frame(date = b_6$date, Links = Links)

# A function to scrape the texts
sp_number = 0
missing <- NULL
for (url in b_6$Links) {
  sp_number = sp_number + 1
    ...
    text <- text %>% 
      html_nodes(xpath = '//p') %>%
      html_text()
    path <- paste0('data/blog_texts/', "b_6_", sp_number, 
                   b_6$date[sp_number], ".txt")
    fileConn<-file(path)
