...
url = 'https://angrybearblog.com'
l_xpath = '//h2//a'
d_xpath = '//article//*[contains(concat( " ", @class, " " ), concat( " ", "postmetadata", " " ))]'
b_21 <- extract_links(url = url,
                         l_xpath = l_xpath,
                         d_xpath = d_xpath)

b_21 <- data.frame(date = b_21$date, Links = b_21$links)

dd <- str_split_fixed(b_21$date, pattern = 'at', n = 2)[, 1]
...
for (url in b_21$Links) {
  sp_number = sp_number + 1
    ...
    text <- text %>% 
      html_nodes(xpath = '//p') %>%
      html_text()
    path <- paste0('data/blog_texts/', "b_21_", sp_number, 
                   b_21$date[sp_number], ".txt")
    fileConn<-file(path)
