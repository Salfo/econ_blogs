...
for (url in b_19$Links) {
  sp_number = sp_number + 1
    ...
    text <- text %>% 
      html_nodes(xpath = '//p') %>%
      html_text()
    path <- paste0('data/blog_texts/', "b_19_", sp_number, 
                   b_19$date[sp_number], ".txt")
    fileConn<-file(path)
